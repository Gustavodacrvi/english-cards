    ...
    <div v-if="firstTime" class="termos">  
      <div class="wrapper">
        <span>
          ...
          <span class="yellow" @click="goToThermsPage">termos de uso</span>
          e a <span class="yellow" @click="goToPrivacyPolicyPage">política de privacidade</span>
        </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    openForgotPasswordPopup() {
      if(this.user.email) {
        this.$store.dispatch('sendResetPassword', this.user.email)
          .then(()=> {
            this.popup = true
            if (this.counter < 1) {
              this.color = '#88E976'
